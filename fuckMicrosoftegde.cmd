...
@set "info2=Если у Вас зависла надпись "ok", то переключитесь на недавно открытое окно и нажмите любую кнопку"
rem Текущая версия
@set Thisversion=1.5.0
REM 1.5.0
...
if %errorlevel%==0 (
cls
Echo Перезапустите программу без прав системы
...
cls
call :EchoColor "                            Вы используете старую версию - %Thisversion%" Green & echo.
call :EchoColor "                                 Актуальная версия - %Version%" Green & echo.
echo. 
call :EchoColor "                                    История изменений:" Green & echo. 
call :EchoColor "                               1.5.0-Добавлен цветной текст" Red & echo.  
call :EchoColor "                          1.4.0-Улучшен запуск от имени системы" Red & echo.   
call :EchoColor "                          1.3.5-Добавление редактивания реестра" Red & echo.   
call :EchoColor "                             1.2.0-Создана защита от дурака" Red & echo.   
call :EchoColor "                            1.1.0-Добавление минимальной версии" Red & echo. 
call :EchoColor "                          1.0.0-Добавление запуска от имени системы" Red & echo. 
call :EchoColor "                                0.5.0-Добавление интерфейса" Red & echo.   
call :EchoColor "                                 0.0.1-Создание программы" Red & echo.   
echo. 
...
call :EchoColor "Unlocker.exe Не обнаружен рядом с программой!" Red & echo.
call :EchoColor "Пожалуйста положите Unlocker.exe рядом с программой" Red & echo.
del /q /s "SID.inf" >nul 2>&1 
pause
exit
)
...
set edgeupdate=.
set WarpJITSvc=.
call :EchoColor "Версия программы:" White & call :EchoColor "%Thisversion%" Green & echo.
echo. 
...
echo. 
echo.Удаление для минимальной версии:
...
echo. 
echo.
echo.Удаление для максимальной версии(в неё входит и минимальная):
...
for  /f "delims=" %%i IN ('2^>nul dir /b "%userprofile%\AppData\Roaming\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar\*edge*" ^| findstr /ic:"Edge"') DO del /q /s "%userprofile%\AppData\Roaming\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar\%%i" >nul 2>&1 && echo ok
echo %info1% & echo %info2%
...
for  /f "delims=" %i IN ('2^>nul REG QUERY "HKEY_CLASSES_ROOT" /f "*edge*"') DO (
...
for  /f "delims=" %%a IN ('2^>nul REG QUERY "HKEY_CLASSES_ROOT\Extensions\ContractId\Windows.AppService\PackageId" /f "Microsoft.MicrosoftEdge*"') DO (
reg delete "%%a" /f  >nul 2>&1 && echo ok
)
for  /f "delims=" %%a IN ('2^>nul REG QUERY "HKEY_CLASSES_ROOT\Extensions\ContractId\Windows.BackgroundTasks\PackageId" /f "Microsoft.MicrosoftEdge*"') DO (
reg delete "%%a" /f  >nul 2>&1 && echo ok
)
for  /f "delims=" %%a IN ('2^>nul REG QUERY "HKEY_CLASSES_ROOT\Extensions\ContractId\Windows.File\PackageId" /f "Microsoft.MicrosoftEdge*"') DO (
reg delete "%%a" /f  >nul 2>&1 && echo ok
)
for  /f "delims=" %i IN ('2^>nul REG QUERY "HKEY_CLASSES_ROOT" /f "*edge*"') DO (
reg delete "%%a" /f  >nul 2>&1 && echo ok
